...
public class BasicViewController {
    ...
    void setTextOfHighScore(String selectTextOfButton, String actualTextOfQuestion){
        ...
        //добавляем очков в зависимости от успешного выбора кнопки
        DatabaseHandler databaseHandler = new DatabaseHandler();
        ResultSet resultSet = databaseHandler.getWord();

        if (selectTextOfButton.length()>0){
            if (databaseHandler.СheckSelection(selectTextOfButton, actualTextOfQuestion)) {
                numberOfHighScore++;
            }
    ...
    @FXML
    ArrayList<String> getWordsFromSQL(String column){
        DatabaseHandler databaseHandler = new DatabaseHandler();
        ResultSet resultSet = databaseHandler.getWord();

        ArrayList<String> wordsFromSQL = new ArrayList<>();

        try{
            while(resultSet.next()) {
                wordsFromSQL.add(resultSet.getString(column));
            }
        }
    ...
    void setTextOnTheScene(String selectTextOfButton){
        ...
        ArrayList<String> englishWords = getWordsFromSQL(Const.WORD);
        ArrayList<String> RUWords = getWordsFromSQL(Const.TRANSLATE_RU);
